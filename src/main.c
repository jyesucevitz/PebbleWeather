...
PBL_APP_INFO(MY_UUID,
             ...
             APP_INFO_WATCH_FACE);
//LEFT TOP WIDE HIGH
#define TIME_FRAME      (GRect(0, 77, 144, 150))
#define DATE_FRAME      (GRect(0, 140, 144, 40))
...
void failed(int32_t cookie, int http_status, void* context) {
	if(cookie == 0 || cookie == WEATHER_HTTP_COOKIE) {
		...
		text_layer_set_text(&weather_layer.highvalue_layer, "??");
...
void handle_init(AppContextRef ctx)
{
    ...
    res_h = resource_get_handle(RESOURCE_ID_TIME_55);
